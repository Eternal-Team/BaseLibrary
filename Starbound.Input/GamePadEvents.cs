...
namespace Starbound.Input
{
	...
	public class GamePadEvents : GameComponent
	{
		...
		{
			return angle > MathHelper.Pi - MathHelper.PiOver4 ||
			       angle <= -MathHelper.Pi + MathHelper.PiOver4;
		}

		private bool InUpDirection(float angle)
		{
			return angle > MathHelper.PiOver2 - MathHelper.PiOver4 &&
		...
		{
			if (LeftTriggerMoved != null)
			{
				LeftTriggerMoved(sender, args);
			}
		}
		...
		{
			if (RightTriggerMoved != null)
			{
				RightTriggerMoved(sender, args);
			}
		}
		...
		{
			if (LeftThumbstickMoved != null)
			{
				LeftThumbstickMoved(sender, args);
			}
		}
		...
		{
			if (RightThumbstickMoved != null)
			{
				RightThumbstickMoved(sender, args);
			}
		}
