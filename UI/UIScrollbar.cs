...
public class UIScrollbar : BaseElement
{
	...
	public UIScrollbar()
	{
		// texture ??= ModContent.Request<Texture2D>("Terraria/Images/UI/Scrollbar").Value;
		// barSliderTexture ??= ModContent.Request<Texture2D>(BaseLibrary.TexturePath + "UI/BarSlider").Value;
	...
	{
		if (args.Button != MouseButton.Left || !HandleRectangle.Contains(args.Position)) return;
